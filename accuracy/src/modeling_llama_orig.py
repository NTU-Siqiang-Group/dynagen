        ...
        **kwargs,
        ...
        if self.enable_quant:
            ...
            for i in range(num_groups):
                ...
                key_max = (
                    torch.max(key_states[:, :, :, start:end], dim=-1, keepdim=True)
                )[0].repeat(1, 1, 1, 64)
                key_min = (
                    torch.min(key_states[:, :, :, start:end], dim=-1, keepdim=True)
                )[0].repeat(1, 1, 1, 64)
                key_delta = key_max - key_min

                value_max = (
                    torch.max(value_states[:, :, :, start:end], dim=-1, keepdim=True)
                )[0].repeat(1, 1, 1, 64)
                value_min = (
                    torch.min(value_states[:, :, :, start:end], dim=-1, keepdim=True)
                )[0].repeat(1, 1, 1, 64)
                value_delta = value_max - value_min

                ### Quantize
                key_states[:, :, :, start:end] = torch.round(
                    ((key_states[:, :, :, start:end] - key_min) / key_delta) * qmax
                )
                value_states[:, :, :, start:end] = torch.round(
                    ((value_states[:, :, :, start:end] - value_min) / value_delta)
                    * qmax
                )

                ### Dequantize
                key_states[:, :, :, start:end] = (
                    key_delta * key_states[:, :, :, start:end] / qmax
                ) + key_min
                value_states[:, :, :, start:end] = (
                    value_delta * value_states[:, :, :, start:end] / qmax
                ) + value_min
