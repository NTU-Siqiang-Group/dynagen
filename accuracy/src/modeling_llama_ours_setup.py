...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if self.partial_weight_q is None:
            ...
            for head in range(int(query.shape[-1] / self.head_dim)):
                start = head * self.head_dim
                end = (head + 1) * self.head_dim
                _, ind = torch.topk(
                    torch.sum(torch.abs(query_states[0, head] @ self.skewing_matrix[head]), dim=-2),
                    int(self.head_dim * self.partial_weight_ratio),
                )
                weight_mask[:, start:end] = weight_mask[:, start:end].scatter(
                    -1, ind.repeat(weight_mask.shape[0], 1), 1
                )
                _, ind = torch.topk(
                    torch.sum(torch.abs(key_states[0, head] @ self.skewing_matrix[head]), dim=-2),
                    int(self.head_dim * self.partial_weight_ratio),
                )
                weight_mask[:, start:end] = weight_mask[:, start:end].scatter(
                    -1, ind.repeat(weight_mask.shape[0], 1), 1
