...
class OptLM:
    def __init__(self, config: Union[str, OptConfig], env: ExecutionEnv, path: str, policy: Policy, args=None):
        ...
        self.computation_policy = get_computation_policy(args.computation_policy)
        ...
        self.cost_tolerance = args.tolerance
    ...
    def compute_layer(self, i, j, k, cpu_delegation=None):
    ...
    def generate(
        ...
        self.prompt_len, gen_len = task.prompt_len, task.gen_len
        self.execute_gen_len = task.cut_gen_len if task.cut_gen_len else task.gen_len

        # Output token ids
        self.output_ids = np.full((len(task.inputs), self.prompt_len + gen_len), self.config.pad_token_id, dtype=np.int32)
        self.stopped = np.zeros((len(task.inputs), 1), dtype=bool)
        self.output_ids[:, :self.prompt_len] = np.asarray(task.inputs)
    ...
    def generation_loop_normal(self, evaluate):
        self.computation_policy.generation_loop_normal(self, evaluate)
    ...
    def generation_loop_debug_multi_batch(self):
        self.computation_policy.generation_loop_debug_multi_batch(self)
...
def add_parser_arguments(parser):
    ...
    parser.add_argument("--cpu-del-percent", type=int, default=0, help="Target percentage of batches computed via CPU delegation.")
    parser.add_argument("--num-prefetch-weight-layers", type=int, default=None,
    ...
    parser.add_argument("--tolerance", type=float, default=1.0, help="Tolerance for how many times to computation costs")
